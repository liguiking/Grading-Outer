			...
			for (int i = 0; i < this.header.fieldArray.length; i++) {
				...
				case 68:
					...
					try {
						...
						recordObjects[i] = calendar.getTime().toString().trim();
				...
				case 70:
					try {
						...
						if ((t_float.length > 0)
							...
							recordObjects[i] = new String(t_float).trim();
						} else {
							recordObjects[i] = null;
						}
					} catch (NumberFormatException e) {
				...
				case 78:
					try {
						...
						if ((t_numeric.length > 0)
							...
							recordObjects[i] = new String(t_numeric).trim();
						} else {
							recordObjects[i] = null;
						}
					} catch (NumberFormatException e) {
				...
				case 76:
					...
					if ((t_logical == 89) || (t_logical == 116)
						...
						recordObjects[i] = Boolean.TRUE.toString().trim();
					} else {
						recordObjects[i] = Boolean.FALSE.toString().trim();
					}
					break;
				case 77:
